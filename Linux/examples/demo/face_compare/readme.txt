一、可执行文件编译
	...
	2. 编译
        ...
        B. 修改demo文件夹下的CMakeLists.txt文件
			...
			只保留add_subdirectory(face_compare),即只编译face_compare demo,默认不编译其余demo
			
二、可执行文件运行
	1、将demo/face_compare/package目录下的对应A311D平台的face_batch_88_A311D.nb文件和face_input.nb输入文件，以及demo/face_compare/aml_face_compare_64可执行文件拷贝平台上，例如拷贝到/data路径下
	2、执行命令：
		./aml_face_compare_64 face_batch_88_A311D.nb face_input.nb
	   Note:  第二个参数 xxx.nb根据板子型号来选取。
	3、运行结果：
		the input type should be 160*160*3*2
		compareScore = 0.000000