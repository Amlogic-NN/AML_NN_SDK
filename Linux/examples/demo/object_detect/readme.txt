一、可执行文件编译
	...
	2. 编译
        ...
        B. 修改demo文件夹下的CMakeLists.txt文件
			...
			只保留add_subdirectory(object_detect),即只编译object_detect demo,默认不编译其余demo
			
二、可执行文件运行
	1、将demo/object_detect/package目录下的对应A311D平台的object_detect_88_A311D.nb文件和416x416x3.jpeg输入文件，以及demo/object_detect/aml_obj_det_64可执行文件拷贝平台上，例如拷贝到/data路径下
	2、执行命令：
		./aml_obj_det_64 object_detect_88_A311D.nb 416x416x3.jpeg
