一、可执行文件编译
	...
	2. 编译
        ...
        B. 修改demo文件夹下的CMakeLists.txt文件
			...
			只保留add_subdirectory(face_recognize),即只编译face_recognize demo,默认不编译其余demo
			
二、可执行文件运行
	1、将demo/face_recognize/package目录下的对应A311D平台的face_recognize_88_A311D.nb文件和112x112x3.jpeg输入文件，以及demo/face_recognize/aml_face_recog_64可执行文件拷贝平台上，例如拷贝到/data路径下
	2、执行命令：
		./aml_face_recog_64 face_recognize_88_A311D.nb 112x112x3.jpeg
	   Note:  第二个参数 xxx.nb根据板子型号来选取。
	3、运行结果：
		the input type should be 112*112*3
		buffer[0] = -0.688203 
		buffer[1] = 1.335924 
		buffer[2] = -0.364343 
		buffer[3] = -1.740749 
