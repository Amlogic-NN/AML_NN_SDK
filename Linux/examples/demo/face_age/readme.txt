一、可执行文件编译
	...
	2. 编译
        ...
        B. 修改demo文件夹下的CMakeLists.txt文件
			...
			只保留add_subdirectory(face_age),即只编译face_age demo,默认不编译其余demo.

二、可执行文件运行
	1、将demo/face_age/package目录下的对应C308X平台的face_age_88_A311D.nb文件和64x64x3.jpeg输入文件，以及demo/face_age/aml_face_age_64可执行文件拷贝平台上，例如拷贝到/data路径下
	2、执行命令：
		./aml_face_age_64 face_age_88_A311D.nb 64x64x3.jpeg
	   Note:  第二个参数 xxx.nb根据板子型号来选取。
	3、运行结果：
		the input type should be 64*64*3
		The age is : 27										
