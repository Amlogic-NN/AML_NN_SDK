一、可执行文件编译
	...
	2. 编译
        ...
        B. 修改demo文件夹下的CMakeLists.txt文件
			...
			只保留add_subdirectory(image_segmentation),即只编译image_segmentation demo,默认不编译其余demo
			
二、可执行文件运行
	1、将demo/image_segmentation/package目录下的对应A311D平台的image_segmentation_88_A311D.nb文件和1024x512x3.jpg输入文件，一个空白的seg_postprocess_output.txt文档，以及demo/image_segmentation/aml_image_seg_64可执行文件拷贝平台上，例如拷贝到/data路径下
	2、执行命令：
		./aml_image_seg_64 image_segmentation_88_A311D.nb 1024x512x3.jpg
	   Note:  第二个参数 xxx.nb根据板子型号来选取。
	3、运行结果：
		the input type should be 512*1024*3	
		结果信息记录保存在seg_postprocess_output.txt文件中