一、可执行文件编译
	...
	2. 编译
        ...
        B. 修改demo文件夹下的CMakeLists.txt文件
			...
			只保留add_subdirectory(face_detection),即只编译face_detection demo,默认不编译其余demo
			
二、可执行文件运行
	1、将demo/face_detection/package目录下的对应A311D平台的face_det_88_A311D.nb文件和640x384x3.jpeg输入文件，以及demo/face_detection/aml_face_det_64可执行文件拷贝平台上，例如拷贝到/data路径下
	2、执行命令：
