...
int run_network(int qnetType, void *qcontext, unsigned char *qrawdata,int fbmode,unsigned char *fbbuf)
{
	...
	switch (qnetType)
	{
		case OBJECT_DETECT:
			...
			obj_detect_out = (obj_detect_out_t*)post_process_all_module(modelType,outdata);


			if (obj_detect_out == NULL)
			{
				printf("post_process_all_module error\n");
				return -1;
			}
...
void* init_network(int argc,char **argv)
{
	...
	if (argv[2][1] == 0)
	{
		netType = (int)(*(char*)argv[2] - '0');
	}
	else
	{
		netType = (int)((*(char*)argv[2] - '0')*10+argv[2][1]-'0');
	}
	if ((netType >= 0 && netType < SUPPORT_NET_NUM) || (netType == CUSTOM_NETWORK))
	{
		printf("nettype is %d\n",netType);
	}
	else
	{
		printf("input nettype is out of range\n");
		return NULL;
	}
	switch (netType)
	{
...
void* net_thread_func(void *args)
{
	...
	if(inmode == AML_IN_PICTURE)
	{
		...
		ret = run_network(netType,context,rawdata,AML_IN_PICTURE,NULL);
	}	
	else if(inmode == AML_IN_VIDEO)
	{
		...
		while (1)
		{
			...
			ret = run_network(netType,context,dst.data,AML_IN_VIDEO,(unsigned char*)img_name);
			index++;	 
		}
